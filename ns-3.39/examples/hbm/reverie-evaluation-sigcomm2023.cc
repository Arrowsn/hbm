                                    ...
                                    long &flowCount, int SERVER_COUNT, int LEAF_COUNT, double START_TIME, double END_TIME, double FLOW_LAUNCH_END_TIME)
{
    ...
    for (int i = 0; i < SERVER_COUNT; i++)
    {
        ...
        while (startTime < FLOW_LAUNCH_END_TIME && startTime > START_TIME)
        {
            ...
            for (int r = 0; r < fan; r++) {
                ...
                query += flowSize;
                std::cout << " RDMA Incast flowCount: " << flowCount << " startTime: " << startTime << " flowSize: " << flowSize << " from " << fromServerIndex << " to " << destServerIndex <<  " fromLeadId " << fromLeafId << " serverCount " << SERVER_COUNT << " leafCount " << LEAF_COUNT <<  std::endl;
                flowCount++;
    ...
    std::cout << "Finished installation of RDMA Incast applications from leaf-" << fromLeafId << std::endl;
}
                           ...
                           long &flowCount, int SERVER_COUNT, int LEAF_COUNT, double START_TIME, double END_TIME, double FLOW_LAUNCH_END_TIME)
{
    for (int i = 0; i < SERVER_COUNT; i++)
    {
        ...
        while (startTime < FLOW_LAUNCH_END_TIME && startTime > START_TIME)
        {
            ...
            std::cout << " RDMA flowCount: " << flowCount << " startTime: " << startTime << " flowSize: " << flowSize << " from " << fromServerIndex << " to " << destServerIndex <<  " fromLeadId " << fromLeafId << " serverCount " << SERVER_COUNT << " leafCount " << LEAF_COUNT <<  std::endl;
            flowCount += 1;
    ...
    std::cout << "Finished installation of RDMA applications from leaf-" << fromLeafId << std::endl;
}
                                  ...
                                  long &flowCount, int SERVER_COUNT, int LEAF_COUNT, double START_TIME, double END_TIME, double FLOW_LAUNCH_END_TIME)
{
    ...
    for (int incastServer = 0; incastServer < SERVER_COUNT; incastServer++)
    {
        ...
        while (startTime < FLOW_LAUNCH_END_TIME && startTime > START_TIME)
        {
            ...
            for (uint32_t txServer = 0; txServer < fan; txServer++) {
                ...
                std::cout << " TCP Incast flowCount: " << flowCount << " startTime: " << startTime << " flowSize: " << flowSize << " from " << txLeaf*SERVER_COUNT + txServer << " to " << incastLeaf*SERVER_COUNT + incastServer <<  " fromLeadId " << txLeaf << " serverCount " << SERVER_COUNT << " leafCount " << LEAF_COUNT <<  std::endl;
    ...
    std::cout << "Finished installation of TCP Incast applications from leaf-" << incastLeaf << std::endl;
}
                           ...
                           long &flowCount, int SERVER_COUNT, int LEAF_COUNT, double START_TIME, double END_TIME, double FLOW_LAUNCH_END_TIME)
{
    ...
    for (int txServer = 0; txServer < SERVER_COUNT; txServer++)
    {
        ...
        while (startTime < FLOW_LAUNCH_END_TIME && startTime > START_TIME)
        {
            ...
            sinkApp.Get(0)->SetAttribute("flowId", UintegerValue(flowCount));
            sinkApp.Get(0)->SetAttribute("senderPriority", UintegerValue(prior));
            std::cout << " TCP flowCount: " << flowCount << " startTime: " << startTime << " flowSize: " << flowSize << " from " << txLeaf*SERVER_COUNT + txServer << " to " << rxLeaf*SERVER_COUNT + rxServer <<  " fromLeadId " << txLeaf << " serverCount " << SERVER_COUNT << " leafCount " << LEAF_COUNT <<  std::endl;
            flowCount += 1;
    ...
    std::cout << "Finished installation of TCP applications from leaf-" << txLeaf << std::endl;
}
...
//     for (int txServer = 0; txServer < 4; txServer++)
//     {
//         double startTime = START_TIME + poission_gen_interval (requestRate);
//         while (startTime < FLOW_LAUNCH_END_TIME && startTime > START_TIME)
//         {
...
//             bulksend->SetStartTime (Seconds(startTime));
//             bulksend->SetStopTime (Seconds (END_TIME));
//             n.Get (txServer)->AddApplication(bulksend);
...
//             sinkApp.Get(0)->SetAttribute("flowId", UintegerValue(flowCount));
//             sinkApp.Get(0)->SetAttribute("senderPriority", UintegerValue(prior));
//             std::cout << " TCP flowCount: " << flowCount << " startTime: " << startTime << " flowSize: " << flowSize << " from " << txServer << " to " << rxServer <<  " fromLeadId " << txLeaf << " serverCount " << SERVER_COUNT << " leafCount " << LEAF_COUNT <<  std::endl;
//             flowCount += 1;
//             sinkApp.Start (Seconds(startTime));
//             sinkApp.Stop (Seconds (END_TIME));
//             sinkApp.Get(0)->TraceConnectWithoutContext("FlowFinish", MakeBoundCallback(&TraceMsgFinish, fctOutput));
...
int main(int argc, char *argv[])
{
    ...
    std::string confFile = "/home/mlx/ns3-datacenter/simulator/ns-3.39/examples/Reverie/config-workload.txt";
    std::string cdfFileName = "/home/mlx/ns3-datacenter/simulator/ns-3.39/examples/Reverie/websearch.csv";
    ...
    std::string alphasFile = "/home/mlx/ns3-datacenter/simulator/ns-3.39/examples/Reverie/alphas"; // On lakewood
